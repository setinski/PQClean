...
main()
{
    ...
    unsigned char       ct[CIPHERTEXT_BYTES], ss[SHARED_SECRET_BYTES], ss1[SHARED_SECRET_BYTES];
    int                 count;
    int                 done;
    unsigned char       pk[PUBLIC_KEY_BYTES], sk[SECRET_KEY_BYTES];
    int                 ret_val;
    ...
    do {
        ...
        if ( (ret_val = PQCLEAN_HQC128_CLEAN_crypto_kem_keypair(pk, sk)) != 0) {
            printf("PQCLEAN_HQC128_CLEAN_crypto_kem_keypair returned <%d>\n", ret_val);
        ...
        if ( (ret_val = PQCLEAN_HQC128_CLEAN_crypto_kem_enc(ct, ss, pk)) != 0) {
            printf("PQCLEAN_HQC128_CLEAN_crypto_kem_enc returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        ...
        if ( (ret_val = PQCLEAN_HQC128_CLEAN_crypto_kem_dec(ss1, ct, sk)) != 0) {
            printf("PQCLEAN_HQC128_CLEAN_crypto_kem_dec returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        
        if ( memcmp(ss, ss1, SHARED_SECRET_BYTES) ) {
            printf("PQCLEAN_HQC128_CLEAN_crypto_kem_dec returned bad 'ss' value\n");
            return KAT_CRYPTO_FAILURE;
        }
