...
static NOINLINE void dec_unpack_ciphertext(DecryptContext *ctx, const uint8_t *salt, const uint8_t *ct) {
...
static NOINLINE void dec_reencrypt(DecryptContext *ctx, const uint8_t *mm) {
...
static NOINLINE void dec_select_message(DecryptContext *ctx) {
    for (size_t i = 0; i < VEC_K_SIZE_BYTES; ++i) {
        ctx -> mc[i] = (ctx -> m[i] & ctx -> result) ^ (ctx -> sigma[i] & ~ctx -> result);
    }
}
